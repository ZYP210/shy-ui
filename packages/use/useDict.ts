...
export const useDict = (useDictStore: () => any, DICT_TYPE_OUT = {}) => {
  /**
   * 数据字典工具类
   */
  ...
  const getDictObj = (dictType: string, value: any) => {
    const dictOptions: DictDataType[] = getDictOptions(dictType)
    dictOptions.forEach((dict: DictDataType) => {
  ...
  const DICT_TYPE = Object.assign(
    {
      ...
      SYSTEM_PERMISSION_MENU: 'system_permission_menu',
      SYSTEM_PERMISSION_RESOURCE: 'system_permission_resource',
      ...
      SYSTEM_TABLE_PERMISSION_BOUND: 'system_table_permission_bound',
